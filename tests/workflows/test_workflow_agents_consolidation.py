...
def test_merge_sync_prs_uploads_machine_readable_report_and_hash_input():
    ...
    assert (
        "report-only mode remains successful" in executor_text
...
def test_autofix_versions_env_is_not_general_template_synced():
    ...
    assert "requirements-dev.lock" in version_sync_text
...
def test_mark_running_uses_same_trusted_app_token_as_summary():
    ...
    for text in (root_loop, consumer_loop):
        ...
        running_end = text.index("  run-codex:", running_start)
...
def test_keepalive_recovery_uses_active_lane_and_forces_only_due_challenges():
    ...
    assert "github-token: ${{ github.token }}" not in update_summary
    assert "trusted_summary_author: process.env.KEEPALIVE_SUMMARY_WRITER" in update_summary
