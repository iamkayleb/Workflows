...
* [`reusable-model-profile-trial.yml`](../../.github/workflows/reusable-model-profile-trial.yml) is a separate immutable, read-only worker for one Sol/Terra/Luna instrumentation arm. It installs exactly Codex CLI 0.144.1, enforces high reasoning and the registry-backed `codex-standard` capacity mapping, and emits a strict quarantine artifact with requested/selected/CLI-reported identity plus source SHA before/after. It never commits, pushes, comments, refreshes auth, or invokes an evaluator; provider-resolved identity is explicitly null.
