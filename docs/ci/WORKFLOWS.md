...
* [`claude-code-review.yml`](../../.github/workflows/claude-code-review.yml) runs Claude Code Review on pull requests when the `CLAUDE_CODE_OAUTH_TOKEN` secret is configured, posting automated review summaries and gracefully skipping with a read-only job when the secret is absent.
