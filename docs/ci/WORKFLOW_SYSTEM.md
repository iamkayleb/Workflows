...
| **Merge Sync PRs** (`maint-71-merge-sync-prs.yml`, maintenance bucket) | `workflow_dispatch`, `workflow_call` | Sole merge/close reconciler for generated sync delivery. Candidate mode validates and uploads plan-bound evidence before merging and supports bounded recovery from trusted already-merged candidate PRs. Required-check discovery falls back to the canonical required-context config when the workflow token cannot read branch protection. | ⚪ Manual/callable | [Merge sync runs](https://github.com/stranske/Workflows/actions/workflows/maint-71-merge-sync-prs.yml) |
