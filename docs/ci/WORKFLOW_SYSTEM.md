...
| Issue / agents automation | Orchestrator dispatch (`workflow_dispatch`, `workflow_call`, `issues`), belt conveyor (`repository_dispatch`, `workflow_run`) | `agents-70-orchestrator.yml`, `agents-71-codex-belt-dispatcher.yml`, `agents-72-codex-belt-worker-dispatch.yml`, `agents-72-codex-belt-worker.yml`, `agents-73-codex-belt-conveyor.yml`, `agents-belt-dispatcher.yml`, `agents-belt-worker.yml`, `agents-belt-conveyor.yml`, `agents-moderate-connector.yml`, `agents-autofix-dispatcher.yml`, `agents-autofix-loop.yml`, `agents-keepalive-loop.yml`, `agents-keepalive-loop-reporter.yml`, `agents-keepalive-branch-sync.yml`, `agents-keepalive-dispatch-handler.yml`, `agents-74-pr-body-writer.yml`, `agents-63-*.yml`, `agents-64-pr-comment-commands.yml`, `agents-64-verify-agent-assignment.yml`, `agents-issue-optimizer.yml`, `agents-guard.yml` | Translate labelled issues into automated work while keeping the protected agents surface locked behind guardrails. |
...
- **Agents Belt Dispatcher** – `.github/workflows/agents-belt-dispatcher.yml`
  agent-agnostic alias that delegates to `agents-71-codex-belt-dispatcher.yml`.
- **Agents Belt Worker** – `.github/workflows/agents-belt-worker.yml`
  agent-agnostic alias that delegates to `agents-72-codex-belt-worker.yml`.
- **Agents Belt Conveyor** – `.github/workflows/agents-belt-conveyor.yml`
  agent-agnostic alias that delegates to `agents-73-codex-belt-conveyor.yml`.
...
| **Health 74 Template Drift** (`health-74-template-drift.yml`, maintenance bucket) | `pull_request`, `push` (agents workflows, templates) | Check for drift between main workflows and consumer repo templates. Warns when templates are significantly out of sync (>50 lines different). | ⚪ Advisory | [Template drift runs](https://github.com/stranske/Workflows/actions/workflows/health-74-template-drift.yml) |
...
| **Verify-to-New-PR Auto-Pilot Continuation** (`agents-verify-to-new-pr-autopilot.yml`, agents bucket) | `workflow_run` (`agents-verify-to-new-pr.yml` completed) | Dispatch auto-pilot (optimize → apply → agent → PR) after follow-up issue creation. | ⚪ Event-driven | [Verify-to-new-pr continuation runs](https://github.com/stranske/Workflows/actions/workflows/agents-verify-to-new-pr-autopilot.yml) |
