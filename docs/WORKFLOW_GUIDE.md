...
- **`agents-verify-to-issue.yml`** — Legacy workflow kept as a disabled stub (`if: false`) so older consumers referencing the filename won’t crash; all functionality lives in the v2 workflow above.
- **`agents-verify-to-new-pr-autopilot.yml`** — Watches the “Create New PR from Verification” workflow and, on success, automatically dispatches `agents-auto-pilot.yml` at the “optimize” step for the generated follow-up issue. It simply reads the artifact output, selects an appropriate token, and fires the dispatcher—no changes needed.
- **`agents-verify-to-new-pr.yml`** — Handles the `verify:create-new-pr` label end-to-end: collects verification comments, reconstructs the original issue context, creates a follow-up issue, emits an artifact with its metadata, and triggers the auto-pilot continuation workflow to resume automation.
