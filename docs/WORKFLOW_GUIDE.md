...
- **`agents-auto-pilot.yml`** — Full issue-to-PR automation pipeline (format → optimize → apply → capability check → PR creation → keepalive → verify). The workflow stayed in service as-is; documentation now clarifies that it orchestrates the reusable agents stack and hands tasks to keepalive without additional per-job token churn.
...
- **`agents-verify-to-new-pr.yml`** — Handles the `verify:create-new-pr` label end-to-end: collects verification comments, reconstructs the original issue context, creates a follow-up issue, and now dispatches `agents-auto-pilot.yml` (optimize step) inline so automation continues without a separate bridge workflow.
